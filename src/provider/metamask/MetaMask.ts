...
import { CarbonEvmChainIDs, EthNetworkConfig, getChangeNetworkParams, Network, NetworkConfigs, RequestArguments, SyncResult } from "@carbon-sdk/constant";
...
import { CARBON_EVM_DEVNET, CARBON_EVM_LOCALHOST, CARBON_EVM_MAINNET, CARBON_EVM_TESTNET, ChangeNetworkParam as MetaMaskChangeNetworkParam } from "../../constant";
...
export class MetaMask extends Eip6963Provider {
  ...
  static async getNetworkParams(network: Network, blockchain: EVMChain = 'Ethereum'): Promise<MetaMaskChangeNetworkParam> {
  ...
  async changeNetworkIfRequired(blockchain: EVMChain, network: CarbonSDK.Network) {
    ...
    try {
      ...
      try {
        const networkParams = await MetaMask.getNetworkParams(network, blockchain)
