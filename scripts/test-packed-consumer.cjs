...
if (!root.Models.registry || legacy.bnOrZero !== legacyJs.bnOrZero || stable.bnOrZero("2").toString() !== "2" || !Array.isArray(abi)) process.exit(1);
`);
...
if (!root.Models.registry || stable("3").toString() !== "3" || stable !== legacy || !MsgBlacklistAddress.fromPartial) process.exit(1);
`);
