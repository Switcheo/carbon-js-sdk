...
export class MetaMask extends Eip6963Provider {
  ...
  static createMetamaskSigner(
    ...
    const signAmino = async (_: string, doc: CarbonTx.StdSignDoc) => {
      ...
      const { sig, signedDoc } = await metamask.signEip712(evmHexAddress, account_number, evmChainId, msgs, fee, memo, sequence);
      const sigBz = Uint8Array.from(Buffer.from(sig, "hex"));
      return {
        signed: signedDoc,
        signature: {
    ...
    const signLegacyEip712 = async (signerAddress: string, doc: CarbonTx.StdSignDoc) => {
      ...
      const { sig, signedDoc } = await metamask.signEip712(evmHexAddress, account_number, chain_id, msgs, fee, memo, sequence, feePayer);
      return {
        signed: signedDoc,
        signature: {
          pub_key: {
  ...
  async verifyConnectedAccount(address: string) {
    const metamaskAPI = await this.getConnectedAPI();
    const accounts = (await metamaskAPI.request({ method: "eth_requestAccounts" })) as string[];
    if (!accounts.find((acc) => acc.toLowerCase() === address?.toLowerCase())) throw new Error(`${address} not connected on Metamask`);
  }
  ...
  async signEip712(
    ...
    const eip712Tx = this.legacyEip712SignMode
      ...
      : constructEIP712Tx(stdSignDoc, metamaskChainId);
