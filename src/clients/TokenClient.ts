import { Coin, Pricing } from "@carbon-sdk/codec/carbon-models";
...
class TokenClient {
  ...
  public readonly tokens: TypeUtils.SimpleMap<Coin.Token> = {};
  public readonly wrapperMap: TypeUtils.SimpleMap<string> = {};
  public readonly poolTokens: TypeUtils.SimpleMap<Coin.Token> = {};
  public readonly cdpTokens: TypeUtils.SimpleMap<Coin.Token> = {};
  ...
  public tokenForId(id: string): Coin.Token | undefined {
  ...
  public tokenForDenom(denom: string): Coin.Token | undefined {
  ...
  public getWrappedTokens(denom: string): Coin.Token[] {
    const result: Coin.Token[] = [];
  ...
  public getWrappedToken(denom: string, blockchain?: BlockchainUtils.Blockchain | BlockchainUtils.BlockchainV2, version = 'V1'): Coin.Token | null {
  ...
  public getSourceToken(denom: string): Coin.Token | null {
    // check if denom is source token
    if (Object.values(this.wrapperMap).includes(denom)) {
      return this.tokens[denom];
    }
  ...
  public getNativeToken(): Coin.Token | undefined {
    return this.tokenForId("swth");
  }

  public getNativeStablecoin(): Coin.Token | undefined {
  ...
  public isCarbonToken(token?: Coin.Token | null): boolean {
    return Boolean(token && (this.isNativeToken(token.denom) || token.bridgeId.eq(0)));
  }

  public getDepositTokenFor(tokenDenom: string, chain: BlockchainUtils.Blockchain | BlockchainUtils.BlockchainV2, version = "V1"): Coin.Token | undefined {
    const networkConfig = this.configProvider.getConfig();
  ...
  public async getAllTokens(): Promise<Coin.Token[]> {
  ...
  public async reloadTokens(): Promise<TypeUtils.SimpleMap<Coin.Token>> {
  ...
  public async getBridges(): Promise<BridgeMap> {
    ...
    const polynetworkBridges = allBridges.bridges.reduce((prev: PolyNetworkBridge[], bridge: Coin.Bridge) => {
  ...
  async matchChainsWithDifferentChainIds(bridges: Coin.Bridge[]): Promise<IbcBridge[]> {
  ...
  public getBridgesFromBridgeId(bridgeId: number): Coin.Bridge[] | IbcBridge[] {
    switch (bridgeId) {
  ...
  public getIbcTokens(): TypeUtils.SimpleMap<Coin.Token> {
    const ibcTokens = Object.values(this.tokens).reduce((prev: TypeUtils.SimpleMap<Coin.Token>, token: Coin.Token) => {
      const newPrev = prev
  ...
  public getPolyNetworkTokens(): TypeUtils.SimpleMap<Coin.Token> {
    const polynetworkTokens = Object.values(this.tokens).reduce((prev: TypeUtils.SimpleMap<Coin.Token>, token: Coin.Token) => {
      const newPrev = prev
  ...
  public getBridgeFromToken(token: Coin.Token | null): Coin.Bridge | IbcBridge | undefined {
  ...
  public getIbcChainIdFromToken(token: Coin.Token | null): string | undefined {
  ...
  processTokenPrices(tokenPrices: Pricing.TokenPrice[]) {
    return tokenPrices.reduce((prevPrices: TypeUtils.SimpleMap<BigNumber>, price: Pricing.TokenPrice) => {
  ...
  async getUSDValuesFromPricingModule() {
    ...
    }) as Pricing.QueryTokenPriceAllResponse;
