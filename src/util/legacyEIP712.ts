...
function getMsgValueType(
  ...
  msgTypeDefinitions: TypeUtils.SimpleMap<TypedDataField[]> = {}
): TypeUtils.SimpleMap<TypedDataField[]> {
      ...
      if (isNonZeroField(fieldValue)) {
        ...
        if (packageName) {
          ...
          if (isGoogleProtobufAnyPackage(packageName, type)) {
            ...
            msgTypeDefinitions[nestedAnyTypeName] = [
...
function getTypeName(name: string, objectName?: string, nestedType: boolean = false, isArray: boolean = false) {
  if (nestedType) {
    return `Type${objectName === "MsgValue" ? "" : objectName}${name
      .split("_")
